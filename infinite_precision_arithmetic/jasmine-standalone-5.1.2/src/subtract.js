...
function validation(arr1, arr2) {
  ...
  for (let i = 1; i < arr1.length; i++) {
    if (!isSingleDigit(arr1[i])) {
      throw new Error(
        `Element at index ${i} in the first array must be a single digit`
      );
    }
  }
  ...
  for (let i = 1; i < arr2.length; i++) {
    if (!isSingleDigit(arr2[i])) {
      throw new Error(
        `Element at index ${i} in the second array must be a single digit`
      );
    }
  }
...
// INPUTS
const sub1 = [1, 2, 3]; //first number
const sub2 = [4, 5, 0]; //second number
